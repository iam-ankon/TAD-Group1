...
const CVEdit = () => {
  ...
  useEffect(() => {
    const fetchCV = async () => {
      try {
        const response = await axios.get(`http://192.168.0.163:8000/api/hrms/api/CVAdd/${id}/`);
        setCv(response.data);
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://192.168.0.163:8000/api/hrms/api/CVAdd/${id}/`, formDataToSubmit, {
