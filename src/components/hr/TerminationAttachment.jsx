...
const TerminationAttachment = () => {
  ...
  const fetchAttachments = async () => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/hrms/api/termination_attachment/?employee_id=${id}`
      );
  ...
  const handleUpload = async () => {
    ...
    try {
      await axios.post(
        "http://127.0.0.1:8000/api/hrms/api/termination_attachment/",
        formData,
        {
  ...
  const handleDelete = async (attachmentId) => {
    ...
    try {
      await axios.delete(
        `http://192.168.0.163:8000/api/hrms/api/termination_attachment/${attachmentId}/`
      );
            ...
            <a
              href={
                attachment.file.startsWith("http")
                  ? attachment.file
                  : `http://192.168.0.163:8000${attachment.file}`
              }
              target="_blank"
